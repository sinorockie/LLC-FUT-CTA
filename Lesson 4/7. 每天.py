...
def test_loop():
        ...
        for sheet_name in carry_df:
            ...
            for i in range(average_days, len(sheet)):
                ...
                if sheet.loc[i-1, 'Carry收益(10日平均)'] > 0:
                    ...
                    sheet.loc[i, '策略收益'] = sheet.loc[i, '策略开仓价'] - sheet.loc[i, '策略结算价'] - (sheet.loc[i, '策略开仓价'] + sheet.loc[i, '策略结算价']) * (0.3/100/100 + 1/100/100)
                else:
                    # 买多
                    ...
                    margin_price = sheet.loc[i, '最优价格'] - 2.5 * sheet.loc[i, 'ATR']
                    sheet.loc[i, '策略结算价'] = margin_price if margin_price > sheet.loc[i, '收盘价'] else sheet.loc[i, '收盘价']
                    sheet.loc[i, '策略收益'] = sheet.loc[i, '策略结算价'] - sheet.loc[i, '策略开仓价'] - (sheet.loc[i, '策略开仓价'] + sheet.loc[i, '策略结算价']) * (0.3/100/100 + 1/100/100)
  ...
  # available_budget = 2978154549.5562663
    ...
    for trade_date in trade_date_series:
        ...
        for key in amount_map.keys():
            ...
            column_index = sheet.columns.get_loc('开仓信号')
            amount = amount_map[key] * sheet.iloc[row_index, column_index]
            column_index = sheet.columns.get_loc('策略收益')
            unit_profit = sheet.iloc[row_index+1, column_index]
            total_profit = unit_profit * amount
            next_date = sheet.iloc[row_index+1, 1]
            df.loc[next_date, (key, '持仓量')] = amount
            df.loc[next_date, (key, '单位收益')] = unit_profit
            df.loc[next_date, (key, '持仓收益')] = total_profit
    ...
    for i in range(len(df)):
        ...
        rows = df.iloc[0:(i+1), :]
        df.iloc[i, 2] = df.iloc[i, 1] / df.iloc[i, 0] * 100
